...
def thchs30(root_path, meta_file, **kwargs):  # pylint: disable=unused-argument
    """https://www.openslr.org/18/"""
    speaker_name = "thchs30"
    wav_files = glob(f"{root_path}/*.wav", recursive=True)
    meta_file_root = os.path.join(os.path.dirname(root_path), 'data')
    items = []
    for wav_file in wav_files:
        text_file_path = os.path.join(meta_file_root, f"{os.path.basename(wav_file)}.trn")
        if os.path.exists(text_file_path) and os.path.exists(wav_file):
            with open(text_file_path, "r", encoding="utf-8") as file_text:
                text = file_text.readlines()[0]
            items.append({"text": text, "audio_file": wav_file, "speaker_name": speaker_name, "root_path": root_path})
    return items
